...
from sociomemory.models.signals import SignalSource
...
class _PrivacyAPI:
    def __init__(self, consent: ConsentManager, neo4j: Neo4jBackend, faiss_dir: str):
    ...
    async def erase(self, child_id: str) -> None:
        ...
        for ext in (".faiss", ".map.json"):
            p = Path(self._faiss_dir) / f"{child_id}{ext}"
            if p.exists():
                p.unlink()
...
class Sociomemory:
    ...
    def __init__(self, config: SociomemoryConfig):
        self._config = config
        self._neo4j = Neo4jBackend(config.neo4j_uri, config.neo4j_user, config.neo4j_password, config.neo4j_database)
    ...
    async def ingest(self, child_id: str, text: str, source: str = "conversation") -> dict:
        ...
        source_enum = SignalSource(source) if source in SignalSource._value2member_map_ else SignalSource.CONVERSATION
    ...
    async def ingest_signal(self, child_id: str, signal_type: str, value: str, confidence: float = 1.0) -> dict:
    ...
    async def get_profile(self, child_id: str) -> SocioProfile:
        ...
        income = await reasoner.infer_income()
        identity = await reasoner.get_behavioral_identity()
        ...
        return SocioProfile(
            child_id=child_id,
            area_type=econ_nodes[0].properties.get("area_type", "unknown") if econ_nodes else "unknown",
            ...
            connectivity_score=transport_nodes[0].properties.get("connectivity_score", 0.5) if transport_nodes else 0.5,
    ...
    async def query(self, child_id: str, question: str) -> str:
        ...
        if self._llm:
            ...
            if embedding:
                import numpy as np
                subgraph = await graph.extract_context_subgraph(np.array(embedding))
                if subgraph.nodes:
